# ============================================================================
# SCRIPT: Targeted Split GBIF MGRS Data Cubes by Ramsar Site Polygons
# ============================================================================
...
# --- Step 2: Move existing CSV files of target sites to temp_holding ---
...
for (cont in continents) {
  ...
  repeat {
    ...
    cat(sprintf("\n- Continent %s, Chunk %d (start line %s)...\n", cont, chunk_num, total_rows_processed + 2))
    
    start_line <- total_rows_processed + 2
    cmd_string <- paste0("tail -n +", start_line, " ", shQuote(cube_path, type = "cmd"), " | head -n ", chunk_size)
    ...
    data_chunk <- tryCatch({
      if (chunk_num == 1) {
        ...
        tryCatch({
          fread(cmd = cmd_string, header = FALSE, col.names = header, encoding = "UTF-8", showProgress = FALSE)
        }, error = function(cmd_err) {
          cat("  [Note] Shell command failed, falling back to pure R fread skip...\n")
...
cat("\n=== Targeted Cube Splitting Process Complete ===\n")
